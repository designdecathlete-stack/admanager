...
text = re.sub(r'const handleSaveNewClient = \(newClient\) => \{[\s\S]*?setView\("clientList"\);\s*\};',
...
text = re.sub(r'const handleAddLog = \(e\) => \{[\s\S]*?setLogAfter\(""\);\s*\};',
...
text = re.sub(r'const handleAddMinute = \(e\) => \{[\s\S]*?setNewMinuteUrl\(""\);\s*\};',
